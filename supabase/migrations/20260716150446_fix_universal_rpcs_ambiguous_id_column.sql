-- Corrige bug introduzido na migration anterior (20260716150136): em plpgsql,
-- as colunas de RETURNS TABLE(...) viram variáveis implícitas no escopo da
-- função. Como a primeira coluna retornada por todas as 4 funções se chama
-- "id", a query "SELECT id INTO v_cliente_id FROM clientes ..." ficava
-- ambígua entre a variável de saída "id" e a coluna real da tabela — erro
-- 42702 em toda chamada. Corrige qualificando a coluna com alias de tabela.
